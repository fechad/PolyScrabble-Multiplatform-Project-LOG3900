...
class ChatService {
  ...
  static List<ChatModel> discussionChannels = [
   ...
   ChatService() {
    ...
    socket.on('message', (data) => {
      if (data.system) {
        discussionChannels[data.roomName].messages.add(ChatMessage(system: true, sender: null, time:  getTime(), message: data.message))
      } else {
        discussionChannels[data.roomName].messages.add(ChatMessage(
          system: false,
          sender: UserModel(username: data.sender.username, email: data.sender.email, icon: data.sender.icon, level: data.sender.level, badges: data.sender.badges, highScore: data.sender.highScore, gamesWon: data.sender.gamesWon),
          time: getTime(),
  ...
  void addDiscussion(String name) {
    ...
    discussionChannels.add(ChatModel(name: name, owner: authenticator.currentUser, activeUsers: 1, icon: "", time:  getTime(), messages: [ChatMessage(sender: null, system: true, time: getTime(), message: "username has started this channel!")], newMessage: 'username has started this channel!'));
    //TODO : envoyer le nom et les détails de la nouvelle convo au  serveur
  }

  void deleteDiscussion(String name) {
    // ignore: list_remove_unrelated_type
    discussionChannels.remove((chat) => chat.name == name && chat.owner == authenticator.currentUser.username);
  }
