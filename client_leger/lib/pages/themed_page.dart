...
class _ThemedPageState extends State<ThemedPage> {
  ...
  final hasBeatenSanta = authenticator.currentUser.badges.contains("Santa");

  Future<void> _showMyDialog(BuildContext context, String name, int difficulty, int time) async {
  ...
  void initState() {
                ...
                children: [
                  Container(
                    ...
                    child: GestureDetector(
                      onTap: () async {
                        await _showMyDialog(context, names[i], difficulty[i], times[i]);
                      },
                      child: Image.asset(
                        hasBeatenSanta || names[i] == "Santa" ? path + names[i] + "Avatar.png" : path + "lock.png",
                      ),
                    )
                  ),
                  Row(
    ...
    for (int i = 0; i<5; i++) {
      if(i>2) hardJV.add(allJV[i]);
      else easyJV.add(allJV[i]);
    }
  }
       ...
       children: [
         ...
         Expanded(child:
            Column(
              ...
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Text('Joueurs virtuels à thème', style: GoogleFonts.nunito(
            textStyle: TextStyle(
