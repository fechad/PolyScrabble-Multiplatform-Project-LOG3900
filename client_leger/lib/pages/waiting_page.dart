...
class WaitingPage extends StatefulWidget {
  const WaitingPage({super.key, required this.timer, required this.isExpertLevel, required this.players});
  final String timer;
  final bool isExpertLevel;
  final List<Player> players;
  @override
  _WaitingPageState createState() => _WaitingPageState(timer: timer, isExpertLevel: isExpertLevel, players: players);
}

class _WaitingPageState extends State<WaitingPage> {
  _WaitingPageState({required this.timer, required this.isExpertLevel, required this.players});
  ...
  connect() {
    socket.on(
        'channelMessage',
            (data) => {
          setState((() => {
            if((data as List<dynamic>)[0]['channelName'] == chatService.getRoomChannel().name) {
            messages = [],
            (data as List<dynamic>).forEach((message) => {
              ...
              if(messages.isNotEmpty && messages[messages.length-1].sender != authenticator.currentUser.username){
    ...
    socketService.on("playerFound", (data) => {
      print('player found'),
      myRoom = gameService.decodeModel(data['room']),
      gameService.room.players.add(gameService.decodePlayer(data['player'])),

      showDialog(barrierDismissible: false, context: context, builder: (context)
      { return
        AlertDialog(
          title: Text("Demande d'accès à la partie"),
          content: Text("Voulez-vous accepter ${gameService.room.players[gameService.room.players.length-1].pseudo} dans la salle de jeu?"),
          actions: [
            TextButton(
                onPressed: () => {
                  gameService.rejectPlayer(gameService.room.players[gameService.room.players.length - 1].pseudo),
    ...
    socketService.on("playerRejected", (room) =>
    {
      print('reject'),
  ...
  leave(){
    if(gameService.room.roomInfo.creatorName == authenticator.currentUser.username) gameService.leaveRoomCreator();
    else gameService.leaveRoomOther();
