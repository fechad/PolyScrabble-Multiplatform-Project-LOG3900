...
class GeneralChatWidget extends StatefulWidget {
  ...
  @override
  _GeneralChatWidgetState createState() =>
      _GeneralChatWidgetState(chat: chat);
}
...
class _GeneralChatWidgetState extends State<GeneralChatWidget> {
    ...
    socket.on(
        'channelMessage',
          ...
          if (messages[messages.length - 1].sender !=
              authenticator.currentUser.username && linkService.currentOpenedChat != chat.name)
            {
              ...
              child: Container(
                child: Row(
                    ...
                    children: [Text(
                      ...
                      chat.name != 'General Chat' && (chat.activeUsers.contains(authenticator.currentUser.username) || chat.owner?.username == authenticator.currentUser.username) ?
                      SizedBox(
                          height: 50,
                          width: 120,
                          child:
                          ElevatedButton(
                              child: Text(chat.owner?.username == authenticator.currentUser.username ? 'Supprimer' : 'Quitter',
                                  style: TextStyle(
                                    color: Colors.white,
                                    fontSize: 18,
                                  )),
                              ...
                              onPressed: () => {
                                showDialog(
                                    ...
                                    builder: (context) {
                                      return Container(
                                        child: AlertDialog(
                                          title: Text(chat.owner?.username == authenticator.currentUser.username ?
                                              "Supprimer la conversation":"Quitter la conversation"),
                                          content: Text(
                                              chat.owner?.username == authenticator.currentUser.username ?
                ...
                Flexible(
                  child: TextField(
                    ...
                    decoration: InputDecoration.collapsed(
                        ...
                        hintText: !chat.activeUsers.contains(authenticator.getCurrentUser().username) && chat.owner?.username != authenticator.getCurrentUser().username
                            && chat.name != 'General Chat' ? "Envoyez un premier message pour rejoindre le canal" : "Aa"),
                  ),
                ),
                Container(
  ...
  void submitMsg(String txt) {
    ...
    if (!chat.activeUsers.contains(authenticator.getCurrentUser().username) && chat.owner?.username != authenticator.getCurrentUser().username) {
  ...
  void leaveChannel() {
    ...
    else chatService.leaveDiscussion(chat.name, authenticator.getCurrentUser().username);
  }
}
